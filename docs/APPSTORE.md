...
#### 3.5.0. Поставить сборку себе через TestFlight

Внутреннее тестирование ревью не требует и доступно сразу после обработки.

В App Store Connect: **TestFlight → Internal Testing → «+»** рядом с
Testers or Groups → создать группу → **Testers → «+» → Add Existing Users**
(владелец аккаунта уже числится пользователем ASC, отметить себя) →
**Builds → «+»** → выбрать сборку.

На айфоне: приложение **TestFlight** из App Store, вход **тем же Apple ID**,
что и в ASC, — приложение появится в списке, **Install**.

Ревью нужно только для внешних тестировщиков. Сборка живёт 90 дней.

**Про покупку:** песочный Apple ID заводить не нужно. В сборках из TestFlight
покупки идут в песочнице под обычным Apple ID тестировщика: деньги
не списываются, лимит снимается по-настоящему. Отдельный Sandbox Account
требуется только для сборок, запущенных напрямую из Xcode.
...
- **Description** и **Keywords** — из раздела 4; **Promotional Text** там же,
  но он необязателен: поле стоит над Description и меняется потом без ревью;
...
Отдельно покупку отправлять не надо и не выйдет: у Apple правило —
**первая покупка приложения подаётся только вместе с версией**. Кнопка
отправки у товара есть, но для первого товара она ответит, что нужна
новая версия.

Порядок: товар доведён до **Ready to Submit** (это готовность, не отправка) →
на странице версии он добавлен в раздел In-App Purchases → кнопка
**Add for Review** вверху страницы версии → **Submit to App Review**.
Уходят оба разом, одной заявкой.

Перед нажатием сверить сводку заявки: там должны быть перечислены
**и сборка, и покупка**. Если покупки нет — её не добавили на странице версии.
Это самая частая осечка: приложение выходит в магазин одно, а кнопка покупки
в нём не работает, потому что товар остался неодобренным.

После релиза правило снимается: следующие товары подаются когда угодно,
без новой версии. Первое ревью — 1–3 дня, изредка до недели.
...
Промотекст — **необязательное** поле, 170 знаков. Живёт на странице версии,
прямо над Description, отдельно для каждого языка (в App Information его нет —
там только название и категории). Ценен тем, что это единственный текст
страницы, который меняется без новой версии и без ревью:
...
Первый релиз прошёл ревью, поэтому дальше порядок короче. В App Store Connect,
на странице приложения: **«+ Version or Platform» → iOS** → номер версии
(например `1.0.1`) → **Create**.

На странице новой версии заполняются только три вещи:

- **What's New in This Version** — обязательно, это единственное новое поле;
- **Build** — загруженная через Transporter сборка с бо́льшим номером
  `CURRENT_PROJECT_VERSION`, чем у всех предыдущих;
- всё остальное — описание, скриншоты, ключевые слова — переносится
  из прошлой версии и правится только при необходимости.

Покупка повторно не подаётся: она уже одобрена. Анкету приватности
и возрастной рейтинг тоже трогать не нужно.
...
- **Перелистывание кнопками громкости.** Было сделано и снято перед 1.0.1:
  правило 2.5.9 запрещает менять назначение системных кнопок. Подробности
  и как вернуть — `docs/ROADMAP.md`.
